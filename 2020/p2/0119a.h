...
void file(struct STATUS amen[],struct STATUS mon[][3][3], int levelup[], char *files[], int *py, int idm[][3], struct STATUS party[], int orh[][3][3]);
void save(char *files[],struct STATUS party[],struct STATUS mon[][3][3], int idm[100][3], struct STATUS amen[]);
    ...
    for(int fpn=0;fpn<max;fpn++){
        ...
        switch(fpn)
        {
            ...
            case  1:while(fscanf(fp1,"%d%d%d%d", &id, &fn, &rn, &mna)!=EOF)
                    {
                        ...
                        orh[fn][rn][mna] = mon[fn][rn][mna].hp;
                    }
                    break;
            ...
            case  4:
                    ...
                    break;
            case  5:for(i=0;fscanf(fp1,"%d%s%d%d%d", &amen[i].id, amen[i].name, &amen[i].pw, &amen[i].hp, &amen[i].ex)!=EOF; i++);
                    break;
                ...
                switch(fil)
                {
                        ...
                        case  3://for(i=0;fprintf(fp2,"%d%s%d%d", &amen[i].id, amen[i].name, &amen[i].pw, &amen[i].hp)!=EOF; i++);
                                for(i=0;s!=0 && party[i].pw>0 && party[i].hp>=0;i++)
                                {
                                        ...
                                        //printf("test-6.%d\n",s);
                                        //printf("test-5.%d\n",i);
                                        fprintf(fp2,"%d\t%s\t%d\t%d\t%d\n", party[i].id, party[i].name, party[i].pw, party[i].hp, party[i].ex);
                                }
                                break;
                        case  4:for(i=0;idm[i][0]!=0;i++)
                                ...
                                break;
                        case  5:for(i=0;party[i].pw>0 && party[i].hp>=0;i++)
                                {
                                        id = party[i].id;
                                        amen[id] = party[i];
                                        //printf("test-1\n");
                                }
                                for(i=0;amen[i].pw>0 && amen[i].hp>=0;i++)
                                {
                                        //printf("test-%d\n",amen[i].ex);
                                        fprintf(fp2,"%d\t%s\t%d\t%d\t%d\n", amen[i].id, amen[i].name, amen[i].pw, amen[i].hp, amen[i].ex);
                                }
                                break;
                        default:break;
                }
